          ...
          TIdSocketHandle *ABinding)
{
	...
	if (clientRequest == "Global"){
		...
		String sql = "SELECT champPlayed,COUNT(*) from noxianconnect.usergames group by champPlayed order by COUNT(*) DESC limit 1";
		...
		pickrateQuery->Active = false;
		String sql = "SELECT userID, champPlayed,COUNT(*) from noxianconnect.usergames WHERE userID =" + clientRequest + " group by champPlayed order by COUNT(*) DESC limit 1";
        pickrateQuery->SQL->Clear();
		pickrateQuery->SQL->Add(sql);
		pickrateQuery->Active = true;
		pickrateQuery->Open();
